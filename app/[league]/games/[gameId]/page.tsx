...
function BoxScoreName({ name, uuid, leagueSlug }: { name: string; uuid?: string; leagueSlug: string }) {
  return (
    <td className="is-name">
      <span className="flex min-w-0 items-center gap-2">
        <PlayerHead uuid={uuid} name={name} size={18} />
        <PlayerProfileLink leagueSlug={leagueSlug} name={name} className="truncate" />
      </span>
    </td>
  );
}
...
type GameParams = { params: Promise<{ league: string; gameId: string }> };

export async function generateMetadata({ params }: GameParams): Promise<Metadata> {
  const { league, gameId: id } = await params;
  ...
  return {
    ...
    alternates: { canonical: `/${league}/games/${gameId}` },
  };
}
          ...
          {[false, true].map((isHome) => {
            ...
            return (
              ...
              >
                ...
                {batting.length > 0 && <BattingTable leagueSlug={league.slug} rows={batting} avatars={avatars} />}
                {pitching.length > 0 && (
                  <div className="border-t border-slate-800">
                    <PitchingTable leagueSlug={league.slug} rows={pitching} avatars={avatars} />
                  </div>
                )}
              </section>
            );
