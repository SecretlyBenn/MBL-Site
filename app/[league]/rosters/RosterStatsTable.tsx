...
const batting: Column[] = [
  { key: "games", label: "G" }, { key: "plateAppearances", label: "PA" }, { key: "atBats", label: "AB" },
  ...
  { key: "sacFlies", label: "SF" }, { key: "leftOnBase", label: "LOB" }, { key: "totalBases", label: "TB" },
  ...
  { key: "fieldingPct", label: "FPCT", format: "rate" },
];
...
export function RosterStatsTable({ rows, kind, avatars, leagueSlug }: {
  ...
  return <div className={styles.tableScroll} tabIndex={0} role="region" aria-label={`${kind} statistics; scroll for more columns`}>
    <table className="data-table">
      <thead><tr>{columns.map((column) => <th key={column.key} scope="col" className={column.key === "battingAverage" || column.key === "putouts" ? styles.groupStart : undefined}
