...
import { Bracket } from "@/app/[league]/standings/Bracket";
...
export async function generateMetadata({
  ...
  return { alternates: { canonical: `/${league}` } };
}
...
export default async function Home({ params }: { params: Promise<{ league: string }> }) {
  ...
  const [standings, leaders, seasonGames, articles] = await Promise.all([
    ...
    Promise.all(
      LEADER_BOARDS.map(async (board) => ({
        ...board,
        rows: await getHistoricalLeaders(board.column, 4, latestSeason?.id, league.id),
      })),
    ),
    ...
    getRecentArticles(league.id, 4).catch(() => []),
  ]);
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)_minmax(0,19rem)]">
          <section className="flex min-w-0 flex-col">
            ...
            <SectionHeader
              ...
              action={
                <SectionLink href={`/${league.slug}/standings`}>
            ...
            {standings.length === 0 || !latestSeason ? (
              ...
              <Bracket leagueSlug={league.slug} games={seasonGames} teams={standings} compact />
            ) : (
          ...
          <section className="min-w-0">
            <SectionHeader
              ...
              action={
                <SectionLink
                  href={`/${league.slug}/statistics/batting${latestSeason ? `?season=${latestSeason.id}` : ""}`}
                >
                  All statistics →
                </SectionLink>
              }
                          ...
                          <tr key={row.playerName}>
                            ...
                            <td>
                              <span className="flex min-w-0 items-center gap-2">
                                ...
                                <PlayerProfileLink leagueSlug={league.slug}
name={row.playerName} className="truncate" />
                              </span>
                            </td>
                            <td>{row.total}</td>
                          </tr>
          ...
          <section className="min-w-0">
            <SectionHeader title="Recent articles" action={<SectionLink href={`/${league.slug}/news`}>All news →</SectionLink>} />
            {articles.length === 0 ? (
              ...
              <ul className="flex flex-col gap-2.5">
                {articles.map((article, index) => (
                  <li key={article.id}>
                    <Link
                      href={`/${league.slug}/news/${article.slug}`}
        ...
        <section className="mt-6">
          <SectionHeader
            title="Seasons"
            action={<SectionLink href={`/${league.slug}/seasons`}>All seasons →</SectionLink>}
          />
          <div className="flex flex-wrap gap-2">
            {seasons.slice(0, 10).map((season) => (
              <Link
                key={season.id}
                href={`/${league.slug}/seasons/${season.id}`}
