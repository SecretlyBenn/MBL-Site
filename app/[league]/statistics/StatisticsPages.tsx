...
export async function IndividualStatisticsPage({ kind, params, searchParams }: { kind: "batting" | "pitching"; params: Params; searchParams: SearchParams }) {
  ...
  const [rows, standings] = await Promise.all([
    getIndividualHistoricalStats(numericSeason, league.id),
...
export async function TeamStatisticsPage({ kind, params, searchParams }: { kind: "batting" | "pitching"; params: Params; searchParams: SearchParams }) {
  const league = await leagueFrom(params);
  const seasons = await getHistoricalSeasons(league.id);
  const requested = (await searchParams).season;
